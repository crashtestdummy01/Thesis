...
CartesianController::update(const rclcpp::Time & time, const rclcpp::Duration & /*period*/) {
  ...
  if (realtime_force_pub_ && realtime_force_pub_->trylock()){
    ...
    msg.wrench.force.x = F_local_force_.x();
    msg.wrench.force.y = F_local_force_.y();
    msg.wrench.force.z = F_local_force_.z();
    
    msg.wrench.torque.x = F_local_torque_.x();
    msg.wrench.torque.y = F_local_torque_.y();
    msg.wrench.torque.z = F_local_torque_.z();
...
CartesianController::on_configure(const rclcpp_lifecycle::State & /*previous_state*/) {
  ...
  realtime_force_pub_->msg_.header.frame_id = params_.end_effector_frame;
  
  tau_ext_.setZero(model_.nv);
  JT_pinv_.setZero(model_.nv, 6);
  F_ext_world_ = Eigen::VectorXd::Zero(6);
  
  zero_ddq_ = Eigen::VectorXd::Zero(model_.nv);
  tau_dyn_ = Eigen::VectorXd::Zero(model_.nv);
  tau_ext_raw_ = Eigen::VectorXd::Zero(model_.nv);
  effort_meas_ = Eigen::VectorXd::Zero(model_.nv);
...
CartesianController::on_activate(const rclcpp_lifecycle::State & /*previous_state*/) {
  ...
  for (size_t i = 0; i < state_interfaces_.size(); i++){
    if (state_interfaces_[i].get_interface_name() == "effort"){
      tau_ext_offset_ = static_cast<int>(i);
      break;
    }
  }
  
  if (tau_ext_offset_ == -1){
