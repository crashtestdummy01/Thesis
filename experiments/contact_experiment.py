...
class SurfaceContactSlidingNode(Node):
    def __init__(self):
        ...
        self.force_topic = 'cartesian_controller/measured_force'
        self.current_pose_topic = '/franka_robot_state_broadcaster/current_pose'

        # Motion & Force Parameter
        self.approach_speed_z = 0.003   # 3 mm/s descent speed
        self.slide_speed_x = 0.008      # 8 mm/s forward sliding speed
        self.slide_distance_x = 0.10    # 10 cm slide distance
        self.contact_threshold_N = 2.5  # Force spike threshold to register contact (N)
        self.target_push_force_z = 4.0  # Constant bias force into surface (N)
        self.surface_offset_z = 0.001   # 1 mm above surface to avoid position spring fighting
    ...
    def control_loop(self):
        ...
        if self.state == 'INIT':
            ...
            self.get_logger().info(
                f"Initialized target pose at Z={self.target_pose.pose.position.z:.3f}m. Starting approach..."
            )
            self.state = 'APPROACH'
